...
public class PswActivity extends Activity {
    ...
    private static String TEMINALPSW ="teminalpsw";
                ...
                if (isSetPsw) {//已设置好密码了。
                    if (isEquals(getHashMap(),points)) {
                        ...
                        if(totalTime==0){
                            ...
                            notice.setText("密码错误,请联系开发者，索要终极密码，电话：17621256872!");
                            return;
                        }
                        notice.setTextColor(Color.RED);
    ...
    private void setTeminalPsw() {
        LinkedHashMap<String, Point> teminalpswpoints = new LinkedHashMap<>();
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                Point point = new Point(mSquarewidth * (0.5f + i),mSquarewidth * (0.5f + j));
                teminalpswpoints.put(i+":"+j,point);
            }
        }
        SharedPreferences.Editor editor = preferences.edit();
        editor.putString(TEMINALPSW, map2String(teminalpswpoints)).commit();
    }
    ...
    public LinkedHashMap<String, Point> getTeminalHashMap() {
        String liststr = preferences.getString(TEMINALPSW, null);
        try {
            return string2Map(liststr);
        } catch (StreamCorruptedException e) {
            e.printStackTrace();
